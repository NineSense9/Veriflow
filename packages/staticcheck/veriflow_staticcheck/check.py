...
def check_workflow(ir: WorkflowIR) -> list[CheckError]:
    ...
    errors.extend(_human_gate(ir, nodes))
    if ir.domain == "compose":
        errors.extend(_cycles(ir))
    return errors
...
def _human_gate(ir: WorkflowIR, nodes: dict[str, Node]) -> list[CheckError]:
    publishes = [
        node_id
        for node_id, node in nodes.items()
    ...
    if not publishes:
        return []
    gates = {nid for nid, node in nodes.items() if node.kind == "human_gate"}
    errors: list[CheckError] = []
    for pub in publishes:
        path = bypass_path(ir, gates, pub)
        if path is not None:
            errors.append(
                CheckError(
...
def _cycles(ir: WorkflowIR) -> list[CheckError]:
    witness = cycle_witness(ir)
    if not witness:
        return []
    return [
        CheckError(
            code="CYCLE_DETECTED",
            message="compose domain forbids directed cycles: " + " → ".join(witness),
            node_id=witness[0] if witness else None,
            witness=witness,
        )
    ]
