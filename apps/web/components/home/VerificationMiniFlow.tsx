...
import { CASE4_STEPS, case4IssueTitle, case4Witness, type Case4Demo } from "@/lib/case4-demo";
    ...
    <aside className={`vf-miniflow${motion ? " vf-miniflow-live" : ""}`} aria-label="CASE 4 验证流">
      ...
      {demo ? (
        <div className="vf-miniflow-finding">
          <p>
            <span className="verdict WA">{demo.issue?.severity || "HIGH"}</span> {demo.issue?.code}
          </p>
          <p>{case4IssueTitle(demo)}</p>
          <p className="caption">Witness {case4Witness(demo)}</p>
        </div>
      ) : (
