    ...
    <Shell>
      <main className={`page wide ${styles.compose}`}>
        <header className={styles.composeHeader}>
          ...
          <p className={styles.eyebrow}>需求编译 / 工作流草案</p>
          <h1>描述出题需求，编译为待核验工作流。</h1>
          <p>输入自然语言需求，系统将抽取目标、边界与时序约束并生成可执行的 IR DAG 结构。</p>
        </header>

        <div className={styles.composeLayout}>
          <form className={styles.inputPanel} onSubmit={compile} aria-busy={busy}>
            <div className={styles.panelHeading}><h2>需求输入</h2><span className={styles.modeChip}>{prefs.aiInterpret ? "AI 语义抽取已开启" : "规则编译"}</span></div>
            ...
            <div className={styles.compileFooter}><span>编译生成结构后，可执行静态多维检查与沙箱时序核验。</span><button className="btn btn-primary" type="submit" disabled={busy || !nl.trim()}>{busy ? "编译中…" : "生成工作流草案"}<span aria-hidden="true">→</span></button></div>
